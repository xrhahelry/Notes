...
Strings get moved and integers get copied.
#+begin_src rust
...
** Slicing
Slicing lets you reference a certain number of elements from a string of characters.
Slices can be further simplified as we can omit the first and last index of the string if our slice begins from the start or goes up to the end of the string.
Omitting both the starting and ending index will reference the whole string.
#+begin_src rust
fn main() {
...
fn main() {
    let user1 = User {
        email: String::from("what@gmail.com"),
        username: String::from("xrhahelry"),
        sign_count: 4,
        active: true,
    };
